...
const router = (request, response) => {
  ...
  } else if (url.indexOf('London') !== -1) {
    console.log('hello received a request');
    apiRequestHandlers.handleLondon(
      request,
      response,
      'http://api.erg.kcl.ac.uk/AirQuality/Daily/MonitoringIndex/Latest/GroupName=London/Json'
    );
  } else if (url.indexOf('GroupName=') !== -1) {
    apiRequestHandlers.handleLondonBoroughs(request, response, url);
  } else {
